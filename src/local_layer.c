...
local_layer make_local_layer(int batch, int h, int w, int c, int n, int size, int stride, int pad, ACTIVATION activation)
{
    ...
    l.weights = (float*)ta_calloc(c*n*size*size*locations, sizeof(float));
    l.weight_updates = (float*)ta_calloc(c*n*size*size*locations, sizeof(float));

    l.biases = (float*)ta_calloc(l.outputs, sizeof(float));
    l.bias_updates = (float*)ta_calloc(l.outputs, sizeof(float));

    // float scale = 1./sqrt(size*size*c);
    float scale = sqrt(2./(size*size*c));
    for(i = 0; i < c*n*size*size; ++i) l.weights[i] = scale*rand_uniform(-1,1);

    l.output = (float*)ta_calloc(l.batch*out_h * out_w * n, sizeof(float));
    l.delta  = (float*)ta_calloc(l.batch*out_h * out_w * n, sizeof(float));
